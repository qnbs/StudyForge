...
3. **Third-Party Integrations:** Malicious responses from external API connections (Zotero, PubMed, arXiv, etc.) that could inject executable code or corrupt local state. Zotero JSON is parsed defensively before Dexie writes; API keys never appear in logs or `.env`.
...
- **Storage Encryption:** Zotero and other BYOK API keys are encrypted at rest with AES-256-GCM in IndexedDB (`secureConfig`). Keys exist in memory only while the vault is unlocked; auto-lock clears the derived session after inactivity.
